//
//  kqWeekActivitiesViewController.m
//  KindergartenPro
//
...
@interface kqWeekActivitiesViewController ()
@property (strong, nonatomic) UITableView *tableView;

@end

@implementation kqWeekActivitiesViewController
...
//    [self.view addSubview:topView];
    ...
    dl.delegate = self;
    if (self.isTeacher) {
        [dl teacherGetWeekInfomationListWithEveryweekTypeId:1];
    }else{
        [dl downLoadWeeklyInfomationWith:@"1" andPageNo:@"0"];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setTapMondayButton_Block:^(NSDictionary *dic) {
        if (dic == nil) {
            [HNAGeneral showAlertViewWithTitle:nil andMessage:@"暂无活动安排！"];
        }else{
            self.hidesBottomBarWhenPushed = YES;
            kqWeekActivitiesDetailViewController *wadVC = [[kqWeekActivitiesDetailViewController alloc] initWithNibName:@"kqWeekActivitiesDetailViewController" bundle:nil];
            wadVC.title = @"活动安排";
            wadVC.dictionary = dic;
            wadVC.isEditMode = self.isTeacher;
